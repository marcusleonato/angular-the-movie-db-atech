...
export class MovieDBService {
  ...
  getMovies(type, options): Observable<Response> {
    type = type || 'popular';
    options.page = options.page || '1'
    return this.http
      .get(`${this.API_BASE}movie/${type}`, this.formatParams(options))
      .pipe(
  ...
  formatMovies(items) {
    const imgUrl = environment.imgUrl,
          imgUrlHDPoster = environment.imgUrlHDPoster,
          imgUrlHDBg = environment.imgUrlHDBg;
    return items.map(item => {
      if (item) {
        item.poster_path = `${imgUrl}${item.poster_path}`;
        item.backdrop_path = `${imgUrl}${item.backdrop_path}`;
        item.poster_hd = `${imgUrlHDBg}${item.poster_path}`;
        item.backdrop_hd = `${imgUrlHDPoster}${item.backdrop_path}`;
  ...
  getGenre(): Observable<Response> {
    const castUrl = `${this.API_BASE}genre/movie/list/credits?api_key=${
      this.API_KEY
      }`;
    return this.http.get(castUrl).pipe(
      map((res: Response) => res),
